...
if (isset($_POST['save']) || isset($_POST['delete']) || isset($_POST['reset']) || isset($_POST['setPin'])) {
	if (isset($project) && $project->getKlant()->getID() != $_SESSION['id']) home();
	if (isset($project)) {
		if (isset($_POST['save'])) {
			if ($_POST['startDatum'] < date("Y-m-d")) $error = true;
			$db->setProject($project->getID(), $_POST['startDatum'], $project->getPincode(), $_POST['tochtID'], $project->getKlant()->getID(), $project->getStatus()->getID(), null);
		...
		if (!is_null($project) && $project->getKlant()->getID() == $_SESSION['id'] && $project->getStatus()->getStatusCode() == 20 && is_null($project->getPINCode())) {
			$trackerID = $db->setTracker(null, intval($_POST['pin']), 0, 0, 0);
			$db->setProject($project->getID(), $project->getStartDatum(), intval($_POST['pin']), $project->getTocht()->getID(), $project->getKlant()->getID(), $project->getStatus()->getID(), $trackerID);
		}
	}
	home();
}
...
switch ($view) {
	case "edit":
		if ($project->getKlant()->getID() != $_SESSION['id']) home();
?>
	...
	?>
		...
		<form action="" method="post">
			...
			<div class="form-group mt-2">
				...
				<input value="<?php echo $project->getKlant()->getNaam(); ?>" type="text" class="form-control" id="klant" disabled>
			</div>
			<div class="form-group mt-2">
				<label for="emailTelefoon">Email/Telefoon:</label>
				<input value="<?php echo $project->getKlant()->getEmail() . " - " . $project->getKlant()->getTelefoon(); ?>" type="text" class="form-control" id="emailTelefoon" disabled>
			</div>
			<div class="form-group mt-2">
				<label for="tocht">Tocht:</label>
				<input value="<?php echo $project->getTocht()->getOmschrijving(); ?>" type="text" class="form-control" id="tocht" disabled>
			</div>
			<br />
	...
	?>
		<h3>Project Klant</h3>
		<?php
		$projecten = $db->getProjectenByKlantID($_SESSION['id']); //$_SESSION['klant_id']
			...
			foreach ($projecten as $project) {
				...
				if ($project->getStatus()->getStatusCode() == 20) {
					if (!is_null($project->getPINCode())) {
						echo "<td><a class='btn btn-primary min-height-0 btn-sm' href='../view?RouteName=". $project->getTocht()->getRoute() . "&PinCode=" . $project->getTracker()->getID() . "," .  $project->getPINCode() . "' target='_blank'>" . str_pad($project->getPINCode(), 4, '0', STR_PAD_LEFT) . "</a>" .
...
if (isset($_GET['setPin'])) {
	$id = $_GET['setPin'];
	$project = $db->getProjectByID($id);
	if (!is_null($project) && $project->getKlant()->getID() == $_SESSION['id'] && is_null($project->getPincode())) {
		?>
			<!-- Modal -->
